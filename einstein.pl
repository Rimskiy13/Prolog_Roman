...
altura(N) :- between(1, 5, N). % Regresa verdadero si la estatura esta entre 1 y 5 %

% Solucion = [casa(Altura, Color, Nacionalidad, Bebida, Cigarro, Mascota)] %
solucion(Solucion) :-
    posibleVecindario(Solucion), validarSolucion(Solucion). %se cumple si la posible solucion se cumple y si la solucion se valida%
...
posibleVecindario(Vecindario) :-
    ...
    % Se le pasa la lista de las casas, se determina si es una posible casa por medio de %
    % altura,color, nacionalidad, bebida, cigarro y mascota. Posterior a eso se valida %
    % la lista de casas % 
...
posibleCasa(Casa) :-
    ...
    validarCasa(Casa).
    %% Se cumple si ña altura, color, nacionalidad, bebida, cigarro, mascota son correctas, ademas de cumplir con la funcioin validarCasa %%


% Validaciones %
validarCasa(Casa) :- forall(restriccionCasa(RestriccionCasa), cumple(RestriccionCasa, Casa)). 
%% Si el contenido de la lista en casa coincide con la funcion RestriccionCasa se aplica la funcion cumple%%
...
%% si la variable condicion es miembro de la lista RestriccionCasa devuelve verdadero y ademas une la lista Casa con Condicion %%
    
cumple(RestriccionCasa, Casa) :- not(afecta(RestriccionCasa, Casa)). %% Es la condicion de parada de la funcion cumple %%
